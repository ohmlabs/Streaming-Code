...
@synthesize mainView;
@synthesize cfgView;
@synthesize workingViewController;
...
- (void)displayMsg:(NSString *)str 
{
 /*   self.mainView.textView.text = [self.mainView.textView.text stringByAppendingString:str];    
    ...
    */
    workingViewController.traceTextView.text = [workingViewController.traceTextView.text stringByAppendingString:str];
}
...
- (void)applicationDidFinishLaunching:(UIApplication *)application 
{
    ...
    [window addSubview:workingViewController.view];
    ...
    NSTimer * t = [[[NSTimer alloc] initWithFireDate:[NSDate date] interval:1.0 target:self selector:@selector(updateLog:) userInfo:nil repeats:YES] autorelease] ;
...
- (void) updateLog:(NSTimer*)theTimer
{
    [self displayMsg:mstr];
    [mstr setString:@""];
}

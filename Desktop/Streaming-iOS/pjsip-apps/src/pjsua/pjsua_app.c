...
void console_app_main(const pj_str_t *uri_to_call)
{
    ...
    pj_status_t ex_pjsua_register_test(char* sipAddress, char* username)
    {
        pjsua_acc_id acc_id;
        pj_status_t status;
        char* SIP_DOMAIN = "sipsorcery.com";
        char* SIP_USER	= "autotune";
        char* SIP_PASSWD =	"password";
        /* Register to SIP server by creating SIP account. */
        {
            pjsua_acc_config cfg;
            
            pjsua_acc_config_default(&cfg);
            cfg.id = pj_str(sipAddress);
            cfg.reg_uri = pj_str("sip:sipsorcery.com");
            cfg.cred_count = 1;
    ...
    pj_status_t ex_pjsua_call_make_call(char *sip_address)
    {
        ...
        if (pjsua_verify_url(sip_address) != PJ_SUCCESS)
        {
            return PJ_EINVAL;
        }       
        
        tmp = pj_str(sip_address);
    ...
    float ex_pjsua_conf_get_tx_level()
    {
        return app_config.speaker_level; 
    }
    
    void ex_pjsua_conf_adjust_tx_level(double level)
    {
        app_config.speaker_level = level;
        pjsua_conf_adjust_tx_level(0, app_config.speaker_level);
    }
    
    float ex_pjsua_conf_get_rx_level()
    {
    ...
    {
        app_config.mic_level = level;
        return pjsua_conf_adjust_rx_level(0, level);
    }
    
    pj_status_t ex_make_bridge_for_all(void)
    {
        pjsua_conf_port_id __call_port_src;
        pjsua_conf_port_id __call_port_dst;
        ...
        for (i = 0; i < max ; i++) 
        {
            if (pjsua_call_is_active(i)) 
            {
                __call_port_src = pjsua_call_get_conf_port(i);  
                
                for (j = i+1; j < max ; j++) 
                {
                    if (pjsua_call_is_active(j)) 
                    {
                        __call_port_dst = pjsua_call_get_conf_port(j);  
                        
                        pjsua_conf_connect( __call_port_src, __call_port_dst );
                        pjsua_conf_connect( __call_port_dst, __call_port_src );
                    }
                }
            }
        }
    ...
    pj_status_t ex_destroy_bridge_for_all(void)
    {
        pjsua_conf_port_id __call_port_src;
        pjsua_conf_port_id __call_port_dst;
            ...
            if (pjsua_call_is_active(i)) 
            {
                ...
                for (j = i+1; j < max ; j++) 
                {
                    if (pjsua_call_is_active(j)) 
                    {
                        ...
                        pjsua_conf_disconnect( __call_port_src, __call_port_dst );
                        pjsua_conf_disconnect( __call_port_dst, __call_port_src );
                    }
                }
            }
        }        
    ...
    pjsua_conf_port_id ex_add_sound_for_all(pjmedia_port * port)
    {
        pjsua_conf_port_id __call_port_src;
        pjsua_conf_port_id __call_port_dst;
        
        /* Create pool for application */
        pj_status_t cp_status = pjsua_conf_add_port(app_config.pool, port, &__call_port_src);
        
        if(cp_status !=  PJ_SUCCESS)
        {
            return -1;            
        }        
    ...
    pj_status_t ex_rem_sound_for_all(pjsua_conf_port_id  __call_port_src)
    {
        ...
        pjsua_conf_remove_port(__call_port_src);
    ...
    pjsua_call_id ex_get_current_call_id(void)
    {
        return current_call;
    }
    
    
    pj_status_t ex_set_codec_sbm()
    {
        pj_str_t id;
        
        pj_status_t status = pjsua_codec_set_priority(pj_cstr(&id,"L16/16000/1"), 245);
     ...
     * Change codec priorities.
     */
    void ex_manage_codec_prio(char *codec, char *prio)
    {
        pj_str_t id;
        int new_prio;
        pj_status_t status;
        ...
        const pj_str_t codec_id = pj_str("L16/16000/1");
        
        status = pjsua_codec_set_priority(&codec_id, 
                                          (pj_uint8_t)new_prio);
